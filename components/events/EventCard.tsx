    ...
    <Link href={`/events/${event.slug.current}`} className="group block h-full no-underline">
      <motion.article
        className={`relative isolate h-full overflow-hidden rounded-[1.6rem] border bg-[#2a2a2a]/90 backdrop-blur-[8px] transition-colors duration-300 ${
          isFeatured
            ? 'border-accent/45 shadow-[0_28px_70px_rgba(255,227,48,0.14)]'
      ...
      >
        ...
        <div className={`relative overflow-hidden ${isFeatured ? 'aspect-[16/8.4]' : 'aspect-[16/10.2]'}`}>
          ...
          <div className="absolute left-4 right-4 top-4 flex items-start justify-between gap-3">
            ...
            {isFeatured && (
              <span className="inline-flex rounded-full border border-accent/50 bg-accent/20 px-3 py-1 text-[0.63rem] font-semibold tracking-[0.14em] uppercase text-accent shadow-[0_8px_26px_rgba(255,227,48,0.25)]">
                Featured
              </span>
            )}
          </div>
