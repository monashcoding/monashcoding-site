...
const handleSendEmail = async (
  ...
  // Used for animating each character in the title separately
  const chars = Array.from(pageTitle);
  ...
  return (
    <RibbonAwareSection
      as="main"
      backgroundClassName=""
      contentClassName="min-h-screen flex flex-col items-center justify-center"
    >
      <div className="w-full py-16 px-8 text-center">
        ...
        <motion.h1
          ...
          initial="hidden"
          animate="visible"
          className="text-[clamp(2.5rem,5vw,4rem)] font-extrabold text-foreground mb-4 leading-[1.1]"
        >
          {chars.map((char, i) => (
            <motion.span 
              key={i} 
